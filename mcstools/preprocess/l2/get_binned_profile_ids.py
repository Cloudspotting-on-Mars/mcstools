...
@click.option(
    "--output-path", default=None, type=str, help="Path to save binned data to"
)
def main(config_file, pds, mcs_data_path, ddr1_profiles_only, output_path):
    ...
    if ddr1_profiles_only:
        ...
        )["Profile_identifier"].aggregate(
            lambda x: list(x.unique()) if len(x) > 0 else []
        )
        if output_path:
        ...
        ddr2 = loader.load(
            ddr="DDR2", profiles=ddr1["Profile_identifier"].unique(), verbose=True
        )
        ddr2.dropna(
        ...
        merged = merged.set_index(filters.bin_config.binned_columns)[
            filters.ddr2_fields
        ]
        if output_path:
            makedirs(output_path)
