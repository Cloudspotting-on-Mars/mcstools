...
@click.option(
    ...
    type=click.DateTime(),
    required=False,
    help="Start and end of time range (ISO format)",
)
@click.option(
    "--lat-range",
    nargs=2,
...
@click.option(
    ...
    type=float,
    required=False,
    help="Minimum and maximum local time (hours)",
)
@click.option(
    "--lon-range",
    nargs=2,
...
@click.option(
    ...
    type=float,
    required=False,
    help="Minimum and maximum altitude",
)
@click.option(
    "--last-az-cmd-range",
    nargs=2,
    type=float,
    required=False,
    help="Minimum and maximum last azimuth command",
)
...
def main(
    ...
    if utc_range:
        ...
        logger.info(
            f"Loaded L1B data for UTC range {utc_range[0].isoformat()} "
            f"to {utc_range[1].isoformat()}, shape: {df.shape}"
        )
    else:
        raise NotImplementedError(
            "UTC range must be specified -- Ls range not implemented yet"
        )
    if lat_range:
        df = df[df["Scene_lat"].between(*lat_range)]
        logger.info(
            f"Restricting L1b data to Scene latitude range {lat_range}"
            f", shape: {df.shape}"
        )
    if lon_range:
        df = df[df["Scene_lon"].between(*lon_range)]
        logger.info(
            f"Restricting L1b data to Scene longitude range {lon_range}"
            f", shape: {df.shape}"
        )
    if lt_range:
        ...
        logger.info(
            f"Restricting L1b data to Local Time range {lt_range}, shape: {df.shape}"
        )
    if alt_range:
        df = df[df["Scene_alt"].between(*alt_range)]
        logger.info(
            f"Restricting L1b data to Scene altitude range {alt_range}"
            f", shape: {df.shape}"
        )
    if last_az_cmd_range:
        df = df[df["Last_az_cmd"].between(*last_az_cmd_range)]
        logger.info(
            "Restricting L1b data to Last Azimuth Command range "
            f"{last_az_cmd_range}, shape: {df.shape}"
        )
