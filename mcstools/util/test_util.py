...
from mcstools.util.geom import scattering_angle, spherical_to_cartesian, mcs_view_angle_to_sc_body_frame
...
def test_mcs_sc_transform():
    ...
    assert pytest.approx(sc_frame_0, .005) == np.array([0.127, 0.860, -0.494])
    assert pytest.approx(sc_frame_1, .005) == np.array([0, 0.939, 0.344])
    assert pytest.approx(sc_frame_2, .005) == np.array([0.933, 0 , 0.360])
